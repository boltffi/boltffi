...
class OpaqueClassTest < DemoTestCase
  ...
  def test_counter_close_releases_handle_and_rejects_access
    counter = Demo::Counter.new(5)
    assert_equal nil, counter.close
    assert_raises(RuntimeError) { counter.get }
    assert_raises(RuntimeError) { counter.close }
  end
