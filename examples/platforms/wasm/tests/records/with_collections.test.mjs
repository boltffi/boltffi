...
export async function run() {
  globalThis.demoCase("case:records.with_collections.polygon.basic");
  ...
  globalThis.demoCase("case:records.with_collections.team.basic");
  const team = demo.makeTeam("devs", ["Ali", "Mia"]);
  assert.deepEqual(demo.echoTeam(team), team);
  assert.equal(demo.teamSize(team), 2);

  globalThis.demoCase("case:records.with_collections.classroom.basic");
  ...
  globalThis.demoCase("case:records.with_collections.tagged_scores.basic");
