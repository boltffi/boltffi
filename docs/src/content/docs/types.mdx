...
These types are matched by name: any signature that spells `Duration`, `SystemTime`, `Uuid` or `Url` crosses as the builtin. So your own `#[data]`, `#[error]`, class, callback or `custom_type!` can't take one of these names. Such a declaration fails to compile with ``a declared type named `Duration` shadows the builtin `Duration` ``. Rename it, for example to `Timeout`.
