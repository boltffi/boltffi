...
setuptools' `build/` directory and the package's `.egg-info` are removed before each wheel build, so every wheel is built from the current sources; `targets.python.wheel.output` must therefore lie outside them. A wheel missing the compiled `_native` extension or the shared library fails the pack.
