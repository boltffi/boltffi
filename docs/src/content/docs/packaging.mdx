...
C packaging emits build-system-neutral artifacts for the current host rather
than a project for a particular build tool:

```bash
boltffi pack c --experimental
```

The output contains `include/<library>.h`, a shared library, and a static
archive under `lib/`. Add those paths to your own compiler or build-system
configuration. The public header requires C11; cross-target, Windows, and C++
consumer support are not yet validated. See [C](/docs/c) for the supported
surface, ownership rules, and shared versus static linking guidance.
