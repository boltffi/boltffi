...
Packaging takes your Rust library and produces artifacts ready for each packaged platform. For packaged targets, a single command handles everything: compiling for each target architecture, generating bindings, and bundling the results into the format each platform expects. C# uses a source-generation flow described below.

`pack` and `generate` mean different things in the CLI. `pack` is for targets where BoltFFI owns the
platform package layout, such as an Apple XCFramework/SwiftPM package, Android jniLibs, Java JNI
output, WASM npm package, or Python wheel. `generate csharp` writes the C# source bindings only;
you build the host Rust `cdylib` and include both the `.cs` files and native library in your .NET
project.
...
<h2 id="c-sharp-generation">C# Generation</h2>

C# uses a source-generation flow because BoltFFI does not create a .NET project or NuGet package.
Build your Rust library as a host dynamic library and generate C# sources:
...
### Using in a .NET Project

Include the generated `.cs` files in your project, allow unsafe blocks, and copy the matching native library next to your application output.
...
<ItemGroup>
  <Compile Include="dist/csharp/*.cs" LinkBase="Generated/BoltFFI" />
</ItemGroup>
```
...
Classes implement `IDisposable`; streams return `IAsyncEnumerable<T>` and can be consumed with `await foreach`.
