...
Each ABI's process builds its own copy of the dependencies and build scripts. Don't point them at a shared `build.build-dir`. Stable Cargo locks the build directory for the whole build, so the ABIs would build one after another again. With nightly's `-Zfine-grain-locking`, concurrent builds that both need to compile the same build scripts can deadlock, as of Cargo 1.100 ([rust-lang/cargo#4282](https://github.com/rust-lang/cargo/issues/4282)).
