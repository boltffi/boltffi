...
C# currently uses a generate-and-integrate flow rather than `pack`:
...
This creates `dist/csharp/` with generated C# source bindings.

`pack` is for targets where BoltFFI creates a platform package, such as an XCFramework,
jniLibs/JNI output, or an npm package. C# already has generated bindings, but BoltFFI leaves the
.NET project or NuGet layout to your app, so the command is `generate csharp`.
...
Build the host dynamic library, generate the C# bindings, and include the generated `.cs` files in your .NET project:
...
Copy the host native library next to your .NET output or make it discoverable by the runtime, then:
