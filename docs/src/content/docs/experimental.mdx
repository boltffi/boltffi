...
| C target | `c` | Experimental (sync surface) |
...
Enabled behind the experimental flag. `boltffi generate c --experimental`
emits a sync-only C header (functions, records, enums, constants, classes,
sync callbacks, error helpers). Async functions, streams, and async callback
completions are reported as unsupported rather than half-rendered.
