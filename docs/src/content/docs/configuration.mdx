...
The `[targets.ruby]` section controls generated Ruby gem output.
...
output = "dist/ruby"
ractor_safe = true
extra_files = ["src/ruby/compat.rb"]
```

`extra_files` lets you ship hand-written Ruby files alongside generated Ruby output. Each path is relative to the selected crate root and must point to a `.rb` file. BoltFFI copies these files under `lib/<crate_stem>/`, adds `require_relative` lines to the generated `lib/<crate_stem>.rb`, and includes the files in the generated gemspec.

Files under a `ruby/` directory preserve the path below that directory. For example, `src/ruby/compat/foo.rb` is emitted as `lib/<crate_stem>/compat/foo.rb` and required as `require_relative "<crate_stem>/compat/foo"`.
