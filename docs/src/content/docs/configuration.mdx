...
The generated `setup.py` declares `python_requires=">=3.10"` by default. Raise it when your package needs a newer Python. The value is written verbatim as a PEP 440 version specifier; pip validates it when the package is built or installed.
...
Declare console scripts under `[targets.python.scripts]`. Each key is the command name, made of ASCII letters, digits, `.`, `_` and `-`. Each value is an entry point in `module:function` form; both sides may be dotted, as in `demo_runtime.cli:App.main`.
