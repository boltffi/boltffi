...
The `[targets.csharp]` section controls C# source generation.

### Output Directory

Generated C# files go into `dist/csharp` by default:
...
The output contains one `.cs` file for the top-level module/runtime helpers and additional `.cs` files for generated records, enums, classes, callbacks, and stream-owning types. Include those files in your .NET project and ship the matching Rust `cdylib` next to your application or in a runtime-discoverable native library path.
