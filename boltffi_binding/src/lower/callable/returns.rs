        ...
        ReturnDef::Value(type_expr) => {
            ...
            if matches!(plan, ReturnPlan::NativeOpaqueRecord { .. }) {
                let error = match owner {
                    ...
                    CallableOwner::Trait(_) | CallableOwner::Closure => {
                        Some(UnsupportedType::NativeOpaqueRecordInCallback)
                    }
...
mod tests {
    ...
    #[test]
    fn closure_owner_direct_opaque_return_is_rejected_as_callback() {
        let mut source = SourceContract::new(PackageInfo::new("demo", None));
        source.records.push(opaque_record());
        let error = lower_return(
            &source,
            CallableOwner::Closure,
            ReturnDef::value(opaque_type()),
        )
        .expect_err("closure opaque return must reject as callback");
