...
impl PrimitiveVector {
    ...
    pub fn populate(&self, storage: &str, source: &str) -> Result<String> {
        Ok(match self.primitive {
            ...
            | Primitive::F64 => {
                format!(
                    "{storage}.ptr.cast<{}>().asTypedList({source}.length).setAll(0, {source});",
                    self.native.native()
                )
            }
    ...
    pub fn writeback(&self, storage: &str, source: &str) -> Result<String> {
        Ok(match self.primitive {
            ...
            | Primitive::F64 => {
                format!(
                    "{source}.setAll(0, {storage}.ptr.cast<{}>().asTypedList({source}.length));",
                    self.native.native()
                )
            }
            _ => return super::super::unsupported("unknown direct-vector primitive"),
    ...
    pub fn copied_from(&self, pointer: &str, length: &str) -> Result<String> {
        let copied = match self.primitive {
            ...
            Primitive::I8 => format!(
                "($$typed_data.Int8List({length})..setRange(0, {length}, {pointer}.cast<$$ffi.Int8>().asTypedList({length})))"
            ),
            Primitive::U8 => format!(
                "($$typed_data.Uint8List({length})..setRange(0, {length}, {pointer}.cast<$$ffi.Uint8>().asTypedList({length})))"
            ),
            Primitive::I16 => format!(
                "($$typed_data.Int16List({length})..setRange(0, {length}, {pointer}.cast<$$ffi.Int16>().asTypedList({length})))"
            ),
            Primitive::U16 => format!(
                "($$typed_data.Uint16List({length})..setRange(0, {length}, {pointer}.cast<$$ffi.Uint16>().asTypedList({length})))"
            ),
            Primitive::I32 => format!(
                "($$typed_data.Int32List({length})..setRange(0, {length}, {pointer}.cast<$$ffi.Int32>().asTypedList({length})))"
            ),
            Primitive::U32 => format!(
                "($$typed_data.Uint32List({length})..setRange(0, {length}, {pointer}.cast<$$ffi.Uint32>().asTypedList({length})))"
            ),
            Primitive::I64 => format!(
                "($$typed_data.Int64List({length})..setRange(0, {length}, {pointer}.cast<$$ffi.Int64>().asTypedList({length})))"
            ),
            ...
            Primitive::F64 => format!(
                "($$typed_data.Float64List({length})..setRange(0, {length}, {pointer}.cast<$$ffi.Double>().asTypedList({length})))"
            ),
            _ => return super::super::unsupported("unknown direct-vector primitive"),
        };
