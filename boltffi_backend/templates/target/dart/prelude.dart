...
_$$BoltFFIBuf _f$encodeUnexpectedCallbackError(Object error) {
  final encoded = $$convert.utf8.encode(error.toString());
  final storage = _$$BoltCallocPtr<$$ffi.Uint8>.alloc(encoded.length + 1);
  try {
    storage.ptr.asTypedList(encoded.length).setAll(0, encoded);
    return _f$boltffi_callback_error(storage.ptr, encoded.length);
  } finally {
    storage.dispose();
  }
}
